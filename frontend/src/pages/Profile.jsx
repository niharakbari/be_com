...
export default function Profile() {
  ...
  return (
    <div className="max-w-2xl text-text-main">
      ...
      <div className="bg-surface rounded-[32px] p-8 shadow-[0_2px_10px_rgb(0,0,0,0.02)] mb-8 border border-border-main text-text-main">
        <div className="flex items-center gap-6 mb-8 pb-8 border-b border-border-main">
          ...
          <div>
            ...
            <p className="text-text-muted flex items-center gap-2 mt-1"><Mail size={16}/> {user?.email}</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-medium mb-2 flex items-center gap-2 text-text-muted">
              <User size={16}/> Username
            </label>
            <input
              type="text"
              name="user_name"
              className="w-full bg-page text-text-main placeholder-text-muted border-none rounded-2xl px-5 py-4 focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)] font-medium"
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="bg-btn-primary text-btn-text rounded-full px-8 py-4 font-semibold hover:bg-btn-primary-hover disabled:opacity-70 transition-colors mt-2"
          >
